...
def func(N, m, v, w, p):
    ...
    for n in range(1, N+1):
        for i in range(m):
            if p[i]>0 and v[p[i]-1]+v[i]<=n:
                dp[i] = max(dp[i], dp[n-v[i]*w[i]-v[p[i]-1]*w[p[i]-1]] + v[i]*w[i]+v[p[i]-1]*w[p[i]-1])
            elif p[i]==0 and v[i] <= n:
                dp[i] = max(dp[i], dp[n-v[i]*w[i]])
